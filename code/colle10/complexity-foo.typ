...
`foo a (b-1)`, puis selon la valeurs de ces appels, soit on refait un 
appel à `foo (a-1) b` ou à `foo a (b-1)`. Chacun de ces appels 
récursifs a une complexité temporelle majorée par $T(n-1)$ car
$(a-1)+b = n-1$ et $a + (b-1) = n-1$.
Par ailleurs, on fait un nombre constant d'autres opérations (assignations, comparaisons, additions, multiplications).
On en déduit donc que la complexité temporelle de `foo a b` est
majorée par $3T(n-1) + O(1)$. On passant au max sur $(a,b) in NN^2$ tel que $a + b = n$, on en déduit que :
$ T(n) <= 3T(n-1) + O(1) $
et donc $T(n) in O(3^n)$.
